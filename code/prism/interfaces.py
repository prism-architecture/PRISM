from LMP import LMP
from utils import get_clock_time, normalize_vector, vec2quat, bcolors, Observation, VoxelIndexingWrapper
import numpy as np
...
EE_ALIAS = ['ee', 'endeffector', 'end_effector', 'end effector', 'gripper', 'hand', 'gripper1']
...
class LMP_interface():
  ...
  # ======================================================
    ...
    #     """Safely get the value of stacked_blocks."""
    #     with self.lock:
    #         return self.task_state['stacked_blocks'] 

    # def set_stacked_blocks(self):
    #     """Safely increment stacked_blocks."""
    #     with self.lock:
    #         self.task_state['stacked_blocks']  += 1  

    # def wait_until_stacked(self):
    #     """Waits until at least one block is stacked, ensuring thread safety."""
    #     while True:
    #         with self.lock:
    #             if self.task_state['stacked_blocks']  >= 1:
    #                 break
    #         time.sleep(0.1)  # Sleep to avoid busy-waiting
    #     print("Block stacked!")

  def get_ee_pos(self,gripper_name):
    return self._world_to_voxel(self._env.get_ee_pos(gripper_name))
  ...
  def detect(self, obj_name):
    ...
    if obj_name.lower() in EE_ALIAS:
      ...
      # print(f"EE_position: {obs_dict['position'] }")
      ...
      z_axis = normalize_vector(obj_normal.mean(axis=0))  # your surface normal

      # Choose a temporary "up" vector not parallel to z
      temp_up = np.array([0, 0, 1]) if abs(z_axis[2]) < 0.99 else np.array([0, 1, 0])

      # x_axis = normalized cross product between temp_up and z
      x_axis = normalize_vector(np.cross(temp_up, z_axis))

      # y_axis = normalized cross product between z and x
      y_axis = normalize_vector(np.cross(z_axis, x_axis))

      obs_dict['normal_right'] = x_axis
      obs_dict['normal_forward'] = y_axis


      # Final basis:
      # x_axis, y_axis, z_axis = right, up, forward
  ...
  def execute(self, movable_obs_func, affordance_map=None, avoidance_map=None, rotation_map=None,
    ...
    # print(f"Mobable object name: {movable_obs['name']}")
    # return
  
    if rotation_map is None:
      rotation_map = self._get_default_voxel_map('rotation', gripper_name=movable_obs['name'])
    if velocity_map is None:
      velocity_map = self._get_default_voxel_map('velocity',gripper_name=movable_obs['name'])
    if gripper_map is None:
      gripper_map = self._get_default_voxel_map('gripper',gripper_name=movable_obs['name'])
    if avoidance_map is None:
      avoidance_map = self._get_default_voxel_map('obstacle',gripper_name=movable_obs['name'])
    ...
    if affordance_map is not None:
      ...
      for plan_iter in range(self._cfg['max_plan_iter']):
        ...
        self._env.increment_planner_calls()
        ...
        if self._cfg['visualize']:
          ...
          step_info['name'] = movable_obs["name"]
          self._env.visualizer.visualize(step_info)

        # execute path
        print(f'Gripper_name: {movable_obs["name"]}'+ f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] start executing path via controller ({len(traj_world)} waypoints){bcolors.ENDC}')
        controller_infos = dict()
        print(f'Gripper_name: {movable_obs["name"]}')
        for i, waypoint in enumerate(traj_world):
          ...
          if np.linalg.norm(movable_obs['_position_world'] - traj_world[-1][0]) <= 0.01:
            print(f'Gripper_name: {movable_obs["name"]}'+ f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] reached last waypoint; curr_xyz={movable_obs['_position_world']}, target={traj_world[-1][0]} (distance: {np.linalg.norm(movable_obs['_position_world'] - traj_world[-1][0]):.3f})){bcolors.ENDC}")
          ...
          if i != 0 and i != len(traj_world) - 1:
            ...
            if np.dot(movable2target, movable2waypoint).round(3) <= 0:
              print(f'Gripper_name: {movable_obs["name"]}'+ f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] skip waypoint {i+1} because it is moving in opposite direction of the final target{bcolors.ENDC}')
              continue
          # execute waypoint
          ...
          if not object_centric and controller_info['mp_info'] == -1:
            print(f'{movable_obs["name"]}'+f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] failed waypoint {i+1} (wp: {waypoint[0].round(3)}, actual: {movable_obs["_position_world"].round(3)}, target: {traj_world[-1][0].round(3)}, start: {traj_world[0][0].round(3)}, dist2target: {dist2target.round(3)}); mp info: {controller_info["mp_info"]}{bcolors.ENDC}')
          else:
            print(f'{movable_obs["name"]}'+f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] completed waypoint {i+1} (wp: {waypoint[0].round(3)}, actual: {movable_obs["_position_world"].round(3)}, target: {traj_world[-1][0].round(3)}, start: {traj_world[0][0].round(3)}, dist2target: {dist2target.round(3)}){bcolors.ENDC}')
        ...
        if distance_transform_edt(1 - _affordance_map)[tuple(curr_pos)] <= 2:
          print(f'{movable_obs["name"]}'+f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] reached target; terminating {bcolors.ENDC}')
          break
    print(f'{movable_obs["name"]}'+f'{bcolors.OKBLUE}[interfaces.py | {get_clock_time()}] finished executing path via controller{bcolors.ENDC}')
    ...
    if not object_centric:
      try:
        ...
        _gripper_map = gripper_map()
        # get last ee pose
        ee_pos_world = self._env.get_ee_pos(movable_obs['name'])
        ee_pos_voxel = self.get_ee_pos(movable_obs['name'])
      ...
      self._env.apply_action(np.concatenate([ee_pose_world, [gripper_state]]),movable_obs['name'])
  ...
  def get_empty_affordance_map(self,gripper_name):
    return self._get_default_voxel_map('target',gripper_name=gripper_name)()  # return evaluated voxel map instead of functions (such that LLM can manipulate it)

  def get_empty_avoidance_map(self,gripper_name):
    return self._get_default_voxel_map('obstacle',gripper_name=gripper_name)()  # return evaluated voxel map instead of functions (such that LLM can manipulate it)
  
  def get_empty_rotation_map(self,gripper_name):
    return self._get_default_voxel_map('rotation',gripper_name=gripper_name)()  # return evaluated voxel map instead of functions (such that LLM can manipulate it)
  
  def get_empty_velocity_map(self,gripper_name):
    return self._get_default_voxel_map('velocity',gripper_name=gripper_name)()  # return evaluated voxel map instead of functions (such that LLM can manipulate it)
  
  def get_empty_gripper_map(self,gripper_name):
    return self._get_default_voxel_map('gripper', gripper_name=gripper_name)()  # return evaluated voxel map instead of functions (such that LLM can manipulate it)
  
  def reset_to_default_pose(self,gripper_name):

    if gripper_name == "gripper":
      in_gripper_pos = self._world_to_voxel(self._env.init_obs.gripper_pose[:3])

    elif gripper_name == "gripper1":
      in_gripper_pos = self._world_to_voxel(self._env.init_obs.gripper_pose_2[:3])


    movable = self.detect(gripper_name)
    affordance_map = self.get_empty_affordance_map(gripper_name)
  
    # 10cm in front of so we add to x-axis

    print(f'In pripper pos: {in_gripper_pos}')

    x, y, z = in_gripper_pos

    
    # print(f'******** Initial gripepr pose, z: {z}')

    affordance_map[x, y, z] = 1
    # movable = lambda: movable
    # affordance_map = lambda: affordance_map
    robot_obs_func = lambda: movable
    affordance_map_func = lambda: affordance_map
    self.execute(robot_obs_func, affordance_map=affordance_map_func)
     
    #  self._env.reset_to_default_pose(gripper_name)
  
  # def wait_until_other(self, gripper_name):
  #   self._env.wait_until_other(gripper_name)
  ...
  def _get_default_voxel_map(self,type='target',gripper_name=None):
    ...
    def fn_wrapper():
      if type == 'target':
        ...
        voxel_map = np.ones((self._map_size, self._map_size, self._map_size)) * self._env.get_last_gripper_action(gripper_name)
      elif type == 'rotation':
        voxel_map = np.zeros((self._map_size, self._map_size, self._map_size, 4))
        voxel_map[:, :, :] = self._env.get_ee_quat(gripper_name)
      else:
...
def setup_LMP(env, general_config, debug=False):
  ...
  # print("*********** Variable names:", list(variable_vars.keys()))


  # allow LMPs to access other LMPs
  lmp_names = [name for name in lmps_config.keys() if not name in ['composer', 'planner', 'config']]
  low_level_lmps = {
      k: LMP(k, 'gripper',lmps_config[k], fixed_vars, variable_vars, debug, env_name, lmp_env=env)
      for k in lmp_names
  }
  ...
  composer = LMP(
      'composer', 'gripper', lmps_config['composer'], fixed_vars, variable_vars, debug, env_name, lmp_env=env
  )
  variable_vars['composer'] = composer

  # creating the LMP that deals w/ high-level language commands
  task_planner = LMP(
      'planner', 'gripper', lmps_config['planner'], fixed_vars, variable_vars, debug, env_name,lmp_env=env
  )
  ...
  lmp_names = [name for name in lmps_config.keys() if not name in ['composer', 'planner', 'config']]
  low_level_lmps_2 = {
      k: LMP(k, 'gripper1', lmps_config[k], fixed_vars, variable_vars, debug, env_name, lmp_env=env)
      for k in lmp_names
  }
  variable_vars_2.update(low_level_lmps_2)

  # creating the LMP for skill-level composition
  composer_2 = LMP(
      'composer', 'gripper1', lmps_config['composer'], fixed_vars, variable_vars_2, debug, env_name, lmp_env=env
  )
  variable_vars_2['composer'] = composer_2

  # creating the LMP that deals w/ high-level language commands
  task_planner_2 = LMP(
      'planner', 'gripper1', lmps_config['planner'], fixed_vars, variable_vars_2, debug, env_name, lmp_env=env
  )
  ...
  lmps_2 = {
      'plan_ui_2': task_planner_2,
      'composer_ui_2': composer_2,
  }
  lmps_2.update(low_level_lmps_2)

  return lmps,lmps_2, lmp_env
