...
class ArmActionMode(object):
    ...
    def set_control_mode(self, robot: Robot, robot_2: Robot):
        robot.arm.set_control_loop_enabled(True)
        robot_2.arm.set_control_loop_enabled(True)
...
class JointPosition(ArmActionMode):
    ...
    def action(self, scene: Scene, action: np.ndarray, gripper_name: str):
        self.action_pre_step(scene, action, gripper_name)
        self.action_step(scene, action, gripper_name)
        self.action_post_step(scene, action, gripper_name)

    def action_pre_step(self, scene: Scene, action: np.ndarray, gripper_name: str):
        assert_action_shape(action, self.action_shape(scene))
    ...
    def action_step(self, scene: Scene, action: np.ndarray, gripper_name: str):
        scene.step()

    def action_post_step(self, scene: Scene, action: np.ndarray, gripper_name: str):
        if gripper_name == "gripper":
            robot = scene.robot
        elif gripper_name == "gripper1":
            robot = scene.robot_2
...
class EndEffectorPoseViaPlanning(ArmActionMode):
    ...
    def _quick_boundary_check(self, scene: Scene, action: np.ndarray, gripper_name: str):
        ...
        relative_to = None if self._frame == RelativeFrame.WORLD else robot.arm.get_tip()
        ...
        if not scene.check_target_in_workspace(pos_to_check):
            raise InvalidActionError('Target outside of workspace.')
    ...
    def _pose_in_end_effector_frame(self, robot: Robot, action: np.ndarray):
        ...
        qw, qx, qy, qz = list(new_rot)
        return [a_x + x, a_y + y, a_z + z, qx, qy, qz, qw]
    ...
    def action(self, scene: Scene, action: np.ndarray, gripper_name: str):
        ...
        self._quick_boundary_check(scene, action, gripper_name)
        ...
        with scene._scene_lock:
            if self._collision_checking:
                if self._robot_shapes is None:
                    self._robot_shapes = robot.arm.get_objects_in_tree(object_type=ObjectType.SHAPE)
                colliding = robot.arm.check_arm_collision()
                if colliding:
                    grasped_objects = robot.gripper.get_grasped_objects()
                    colliding_shapes = [
                        s for s in scene.pyrep.get_objects_in_tree(object_type=ObjectType.SHAPE)
                        if s.is_collidable() and s not in self._robot_shapes
