...
class Environment(object):
    ...
    def __init__(self,
        ...
        print(f'Save directory from Environment: {self._save_directory}')
    ...
    def launch(self):
        ...
        if self._robot_setup != 'panda':
            ...
            arm, gripper = arm_class(count=0), gripper_class(count=0)
            arm_2, gripper_2 = arm_class(count=1), gripper_class(count=1)

        self._robot = Robot(arm, gripper)
        self._robot_2 = Robot(arm_2, gripper_2)
        

        if self._randomize_every is None:
            self._scene = Scene(
                self._pyrep, self._robot, self._robot_2, self._obs_config, self._robot_setup,self._save_directory)

            # self._scene = Scene(
            #     self._pyrep, self._robot, self._obs_config, self._robot_setup)
        else:
            self._scene = DomainRandomizationScene(
                self._pyrep, self._robot, self._robot_2, self._obs_config, self._robot_setup,
                self._randomize_every, self._frequency,
                self._visual_randomization_config,
                self._dynamics_randomization_config)
        self._action_mode.arm_action_mode.set_control_mode(self._robot, self._robot_2)
    ...
    def get_task(self, task_class: Type[Task]) -> TaskEnvironment:
        ...
        self._scene.unload()
        task = task_class(self._pyrep, self._robot, self._robot_2)
        self._prev_task = task
        return TaskEnvironment(
            self._pyrep, self._robot, self._robot_2, self._scene, task,
